...
def transcribe_audio_with_diarization(audio_data, segments, sampling_rate=16000, model = whisper.load_model("base")):
    """
...
def summarize_chunks(chunks, model, tokenizer, max_length = 130, length_penalty = 2.0):
    summaries = []
    for chunk in chunks:
...
def summarize_speaker(model, transcribed_dict, tokenizer, summary_length = 250):
    summary_dict={}
    for key in transcribed_dict:
        ...
        summaries = summarize_chunks(chunks, model, tokenizer, max_length = summary_length)
